...
public class TreeIntercptor extends EmptyInterceptor implements
	...
	public boolean onSave(Object entity, Serializable id, Object[] state,
		...
		if (entity instanceof HibernateTree) {
			...
			if (parentId != null) {
				...
				myPosition = ((Number) session.createQuery(hql)
	...
	private boolean updateParent(HibernateTree tree, HibernateTree preParent,
			HibernateTree currParent) {
		// 都为空、或都不为空且相等时，不作处理
		...
		session.createQuery(hql).setParameter("offset", offset)
				...
				.setParameter("nodeRgt", nodeRgt).executeUpdate();
		if (log.isDebugEnabled()) {
