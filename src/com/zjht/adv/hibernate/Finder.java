...
public class Finder {
	...
	/**
	 * 获得查询数据库记录数的hql语句。(带group by的hql)
	 * 
	 * @return
	 */
	public String getGroupRowCountHql(String groupbyProperty) {
		String hql = hqlBuilder.toString();
		...
		index=rowCountHql.indexOf("group by");
		if (index>0) {
			rowCountHql=rowCountHql.substring(0,index);
			if (projectionHql.indexOf("select") == -1) {
				rowCountHql="select count(distinct "+groupbyProperty+")"+rowCountHql;
			}else{
				rowCountHql=projectionHql.replace("select", "select count(") + ") "+rowCountHql;
			}
			return rowCountHql;
		}else{
			if (projectionHql.indexOf("select") == -1) {
	...
	public static void main(String[] args) {
		Finder find = Finder
				.create("select * FROM t_coupon_consume_record group by order_no");
