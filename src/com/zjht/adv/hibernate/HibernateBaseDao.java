package com.zjht.adv.hibernate;
...
import com.zjht.adv.util.MyBeanUtils;
...
public abstract class HibernateBaseDao<T, ID extends Serializable> {
	 ...
	 * @param pageSize
	 *            每页条数
	 * @param groupbyPropery
	 *            分组字段
	 * @return
	 */
	@SuppressWarnings("rawtypes")
	protected Pagination findByGroup(Finder finder, int pageNo, int pageSize,String groupbyPropery) {
		int totalCount = countGroupQueryResult(finder,groupbyPropery);
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (totalCount < 1) {
			p.setList(new ArrayList());
			return p;
	 ...
	 * @return
	 */
	protected int countGroupQueryResult(Finder finder,String groupbyProperty) {
		Query query = getSession().createQuery(finder.getGroupRowCountHql(groupbyProperty));
		finder.setParamsToQuery(query);
		if (finder.isCacheable()) {
			query.setCacheable(true);
		}
